...
class HDF5ShortWriter extends HDF5ShortReader implements IHDF5ShortWriter
{
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        if (features.requiresChunking())
        {
            return baseWriter.createDataSet(objectPath, features.isSigned() ? H5T_STD_I16LE : H5T_STD_U16LE,
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        return baseWriter.createDataSetTemplate(
                features.isSigned() ? H5T_STD_I16LE : H5T_STD_U16LE, features, new long[]
                    { size }, new long[]
                    { blockSize }, 2);
    }
        ...
        final ICallableWithCleanUp<Void> writeRunnable = new ICallableWithCleanUp<Void>()
            {
                ...
                public Void call(ICleanUpRegistry registry)
                {
                    ...
                    final long memorySpaceId =
                            baseWriter.h5.createSimpleDataSpace(blockDimensions, registry);
                    H5Dwrite(dataSet.getDataSetId(), H5T_NATIVE_INT16, memorySpaceId, dataSpaceId,
                            H5P_DEFAULT, data);
                    return null; // Nothing to return.
                }
            };
