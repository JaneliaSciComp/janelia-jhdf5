...
class HDF5UnsignedIntWriter extends HDF5UnsignedIntReader implements IHDF5IntWriter
{
                ...
                public Void call(ICleanUpRegistry registry)
                {
                    final long dataSetId =
                            baseWriter.createDataSetFromTemplate(objectPath,
                                    template, registry);
                    H5Dwrite(dataSetId, H5T_NATIVE_UINT32, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
                    return null; // Nothing to return.
                }
            };
        baseWriter.runner.call(writeRunnable);
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        if (features.requiresChunking())
        {
            return baseWriter.createDataSet(objectPath, H5T_STD_U32LE,
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        return baseWriter.createDataSetTemplate(
                H5T_STD_U32LE, features, new long[]
                    { size }, new long[]
                    { blockSize }, 4);
    }
    ...
    @Override
    public void writeArrayBlock(final HDF5DataSet dataSet, final int[] data,
            final long blockNumber)
    {
    ...
    @Override
    public void writeArrayBlockWithOffset(final HDF5DataSet dataSet, final int[] data,
            final int dataSize, final long offset)
    {
        assert dataSet != null;
        assert data != null;
        ...
        final ICallableWithCleanUp<Void> writeRunnable = new ICallableWithCleanUp<Void>()
            {
                ...
                public Void call(ICleanUpRegistry registry)
                {
                    ...
                    final long memorySpaceId =
                            baseWriter.h5.createSimpleDataSpace(blockDimensions, registry);
                    H5Dwrite(dataSet.getDataSetId(), H5T_NATIVE_UINT32, memorySpaceId, dataSpaceId,
                            H5P_DEFAULT, data);
                    return null; // Nothing to return.
                }
            };
    ...
    public void writeMDArraySlice(HDF5DataSet dataSet, MDIntArray data, IndexMap boundIndices)
    {
        ...
        MatrixUtils.createFullBlockDimensionsAndOffset(data.dimensions(), null, boundIndices,
                fullRank, fullBlockDimensions, fullOffset);
        writeMDArrayBlockWithOffset(dataSet, new MDIntArray(data.getAsFlatArray(),
                fullBlockDimensions), fullOffset);
    }
            ...
            HDF5IntStorageFeatures features)
    {
        ...
        baseWriter.checkOpen();
        return baseWriter.createDataSetTemplate(
                H5T_STD_U32LE, features, dimensions, MDArray.toLong(blockDimensions), 4);
    }
    ...
    public void writeMDArrayBlock(final HDF5DataSet dataSet, final MDIntArray data,
            final long[] blockNumber)
    {
        assert blockNumber != null;
    ...
    public void writeMDArrayBlockWithOffset(final HDF5DataSet dataSet, final MDIntArray data,
            final long[] offset)
    {
        assert dataSet != null;
        assert data != null;
    ...
    @Override
    public void writeSlicedMDArrayBlockWithOffset(HDF5DataSet dataSet, MDIntArray data,
            long[] offset, IndexMap boundIndices)
    {
        assert dataSet != null;
        assert data != null;
