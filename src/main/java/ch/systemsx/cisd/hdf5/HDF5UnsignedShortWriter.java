...
class HDF5UnsignedShortWriter extends HDF5UnsignedShortReader implements IHDF5ShortWriter
{
                ...
                public Void call(ICleanUpRegistry registry)
                {
                    final long dataSetId =
                            baseWriter.createDataSetFromTemplate(objectPath,
                                    template, registry);
                    H5Dwrite(dataSetId, H5T_NATIVE_UINT16, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
                    return null; // Nothing to return.
                }
            };
        baseWriter.runner.call(writeRunnable);
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        if (features.requiresChunking())
        {
            return baseWriter.createDataSet(objectPath, H5T_STD_U16LE,
                features, new long[] { 0 }, new long[] { size }, 2);

        } else
        {
            return baseWriter.createDataSet(objectPath, H5T_STD_U16LE,
                features, new long[] { size }, null, 2);
        }
    }
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        return baseWriter.createDataSetTemplate(
                H5T_STD_U16LE, features, new long[]
                    { size }, new long[]
                    { blockSize }, 2);
    }
    ...
    @Override
    public void writeArrayBlockWithOffset(final HDF5DataSet dataSet, final short[] data,
            final int dataSize, final long offset)
    {
        assert dataSet != null;
        assert data != null;
        ...
        final ICallableWithCleanUp<Void> writeRunnable = new ICallableWithCleanUp<Void>()
            {
                ...
                public Void call(ICleanUpRegistry registry)
                {
                    ...
                    final long memorySpaceId =
                            baseWriter.h5.createSimpleDataSpace(blockDimensions, registry);
                    H5Dwrite(dataSet.getDataSetId(), H5T_NATIVE_UINT16, memorySpaceId, dataSpaceId,
                            H5P_DEFAULT, data);
                    return null; // Nothing to return.
                }
            };
    ...
    public void writeMDArraySlice(HDF5DataSet dataSet, MDShortArray data, IndexMap boundIndices)
    {
        ...
        MatrixUtils.createFullBlockDimensionsAndOffset(data.dimensions(), null, boundIndices,
                fullRank, fullBlockDimensions, fullOffset);
        writeMDArrayBlockWithOffset(dataSet, new MDShortArray(data.getAsFlatArray(),
                fullBlockDimensions), fullOffset);
    }
        ...
        final ICallableWithCleanUp<HDF5DataSet> createRunnable = new ICallableWithCleanUp<HDF5DataSet>()
            {
                ...
                public HDF5DataSet call(ICleanUpRegistry registry)
                {
                    if (features.requiresChunking())
                    {
                        final long[] nullDimensions = new long[dimensions.length];
                        return baseWriter.createDataSet(objectPath, H5T_STD_U16LE, 
                                features, nullDimensions, MDArray.toLong(dimensions), 2);
                    } else
                    {
                        return baseWriter.createDataSet(objectPath, H5T_STD_U16LE, 
                                features, MDArray.toLong(dimensions), null, 2);
                    }
                }
            };
        ...
        final ICallableWithCleanUp<HDF5DataSet> createRunnable = new ICallableWithCleanUp<HDF5DataSet>()
            {
                ...
                {
                    return baseWriter.createDataSet(objectPath, H5T_STD_U16LE, 
                            features, dimensions, MDArray.toLong(blockDimensions), 2);
                }
            };
        return baseWriter.runner.call(createRunnable);
    }
            ...
            HDF5IntStorageFeatures features)
    {
        ...
        baseWriter.checkOpen();
        return baseWriter.createDataSetTemplate(
                H5T_STD_U16LE, features, dimensions, MDArray.toLong(blockDimensions), 2);
    }
    ...
    public void writeMDArrayBlock(final HDF5DataSet dataSet, final MDShortArray data,
            final long[] blockNumber)
    {
        assert blockNumber != null;
    ...
    public void writeMDArrayBlockWithOffset(final HDF5DataSet dataSet, final MDShortArray data,
            final long[] offset)
    {
        assert dataSet != null;
        assert data != null;
