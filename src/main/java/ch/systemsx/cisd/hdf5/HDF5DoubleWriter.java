...
class HDF5DoubleWriter extends HDF5DoubleReader implements IHDF5DoubleWriter
{
            ...
            final HDF5FloatStorageFeatures features)
    {
        ...
        if (features.requiresChunking())
        {
            return baseWriter.createDataSet(objectPath, H5T_IEEE_F64LE,
            ...
            final HDF5FloatStorageFeatures features)
    {
        ...
        return baseWriter.createDataSetTemplate(
                H5T_IEEE_F64LE, features, new long[]
                    { size }, new long[]
                    { blockSize }, 8);
    }
    ...
    @Override
    public void writeArrayBlock(final HDF5DataSet dataSet, final double[] data,
            final long blockNumber)
    {
    ...
    @Override
    public void writeArrayBlockWithOffset(final HDF5DataSet dataSet, final double[] data,
            final int dataSize, final long offset)
    {
        assert dataSet != null;
        assert data != null;
        ...
        final ICallableWithCleanUp<Void> writeRunnable = new ICallableWithCleanUp<Void>()
            {
                ...
                public Void call(ICleanUpRegistry registry)
                {
                    ...
                    final long memorySpaceId =
                            baseWriter.h5.createSimpleDataSpace(blockDimensions, registry);
                    H5Dwrite(dataSet.getDataSetId(), H5T_NATIVE_DOUBLE, memorySpaceId, dataSpaceId,
                            H5P_DEFAULT, data);
                    return null; // Nothing to return.
                }
            };
    ...
    public void writeMDArraySlice(HDF5DataSet dataSet, MDDoubleArray data, IndexMap boundIndices)
    {
        ...
        MatrixUtils.createFullBlockDimensionsAndOffset(data.dimensions(), null, boundIndices,
                fullRank, fullBlockDimensions, fullOffset);
        writeMDArrayBlockWithOffset(dataSet, new MDDoubleArray(data.getAsFlatArray(),
                fullBlockDimensions), fullOffset);
    }
            ...
            HDF5FloatStorageFeatures features)
    {
        ...
        baseWriter.checkOpen();
        return baseWriter.createDataSetTemplate(
                H5T_IEEE_F64LE, features, dimensions, MDArray.toLong(blockDimensions), 8);
    }
    ...
    public void writeMDArrayBlock(final HDF5DataSet dataSet, final MDDoubleArray data,
            final long[] blockNumber)
    {
        assert blockNumber != null;
    ...
    public void writeMDArrayBlockWithOffset(final HDF5DataSet dataSet, final MDDoubleArray data,
            final long[] offset)
    {
        assert dataSet != null;
        assert data != null;
