...
class HDF5UnsignedByteWriter extends HDF5UnsignedByteReader implements IHDF5ByteWriter
{
                ...
                public Void call(ICleanUpRegistry registry)
                {
                    final long dataSetId =
                            baseWriter.createDataSetFromTemplate(objectPath,
                                    template, registry);
                    H5Dwrite(dataSetId, H5T_NATIVE_UINT8, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
                    return null; // Nothing to return.
                }
            };
        baseWriter.runner.call(writeRunnable);
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        if (features.requiresChunking())
        {
            return baseWriter.createDataSet(objectPath, H5T_STD_U8LE,
            ...
            final HDF5IntStorageFeatures features)
    {
        ...
        return baseWriter.createDataSetTemplate(
                H5T_STD_U8LE, features, new long[]
                    { size }, new long[]
                    { blockSize }, 1);
    }
